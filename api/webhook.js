...
// ============================================================
// DEDUP — simpan update_id yang sudah diproses (in-memory)
// Di Vercel setiap request bisa jadi instance baru, jadi
// kita pakai pendekatan: proses cepat + return 200 duluan
// ============================================================
...
function isKoreksiRequest(text) {
  ...
  return kw.some(k => lo.includes(k));
}
...
async function konfirmasiKoreksi(chatId, rowIndex, nominalBaru) {
  try {
    ...
    await row.save();
    await recalculateSaldo(sheet, rows, rowIndex - 2);
    await sendMessage(chatId,
...
async function recalculateSaldo(sheet, rows, fromIdx) {
